...
def problem3_1(txtfilename):
    ...
    data = f.read()
    print(data, end="")
    print("\n\nThere are", len(data), "letters in the file.")
    f.close()
...
def problem3_2(collection):
    """ Takes a collection (list, tuple, or string) and prints its items one per line """
...
def problem3_3(month, day, year):
    ...
    month_name = months[month - 1]  # month-1 because tuple is 0-indexed
    date_str = month_name + " " + str(day) + ", " + str(year)
    print(date_str)
#%%
"""
Problem 3_4:
...
def problem3_4(mon, day, year):
    ...
    date_str = str(month_num) + "/" + str(day) + "/" + str(year)
    print(date_str)
...
def problem3_7(csv_pricefile, flower):
    with open(csv_pricefile, 'r') as file:
...
if __name__ == "__main__":
    ...
    problem3_4("July",17, 2016)
    problem3_5("james")
    problem3_6("humptydumpty.txt", "humptylength.txt")
    problem3_7("flowers.csv","alyssum")